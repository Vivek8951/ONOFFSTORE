...
export default function AdminPage() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      fetchArchive();
      const timer = setInterval(() => fetchArchive(true), 30000); // 30s auto-sync
  ...
  return (
    <div className="min-h-screen bg-[var(--indian-cream)] text-gray-900 font-sans pb-32 md:pb-0 overflow-x-hidden">
      ...
      {/* SYNC INDICATOR */}
      {isSyncing && (
         <div className="fixed top-28 left-1/2 -translate-x-1/2 z-[500] bg-[var(--indian-gold)] text-[var(--indian-midnight)] px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-[0.3em] shadow-2xl animate-fade-in-up flex items-center gap-3">
            <span className="w-2 h-2 bg-white rounded-full animate-pulse shadow-[0_0_10px_white]"></span>
            ATELIER REALTIME SYNC ACTIVE
         </div>
      )}
                    ...
                    <div key={item.id} className="glass-card rounded-[40px] overflow-hidden group hover:shadow-2xl transition-all border border-gray-100/50">
                       <div className="aspect-square relative overflow-hidden bg-gray-50">
                          ...
                          <div className="absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                             <button onClick={() => { setIsEditingProduct(item.id); setIsAddingProduct(true); setNewProduct({...item, price: item.price.toString()}); }} className="bg-white/90 p-4 rounded-full shadow-xl text-lg">✎</button>
                          </div>
                 ...
                 {orders.map(order => (
                    <div key={order._id} className="glass-card p-10 rounded-[40px] border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-10 hover:shadow-xl transition-all">
                       ...
                       <div className="flex items-center gap-8">
                          ...
                          <button onClick={() => setSelectedOrder(order)} className="bg-[var(--indian-midnight)] text-white p-4 rounded-full"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="9 18 15 12 9 6"></polyline></svg></button>
                       </div>
                    </div>
                 ))}
              </div>
      ...
      {selectedOrder && (
         <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 md:p-12">
            <div className="absolute inset-0 bg-[#0a0a0b]/90 backdrop-blur-md" onClick={() => setSelectedOrder(null)}></div>
            <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto no-scrollbar bg-[var(--indian-cream)] rounded-[60px] p-10 md:p-20 shadow-2xl border-2 border-[var(--indian-gold)]/30 animate-fade-in-up">
               <div className="flex flex-col md:flex-row justify-between gap-12 mb-16 pb-12 border-b border-gray-100">
                  <div>
                     <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-[var(--indian-gold)] mb-4 italic">Purchase Archive</p>
                     <h2 className="text-4xl md:text-6xl font-serif font-bold italic tracking-tighter uppercase">{selectedOrder.user}</h2>
                  </div>
                  <div className="bg-white p-8 rounded-[40px] border border-gray-100 flex flex-col gap-6">
                     ...
                     <div className="flex flex-wrap gap-2">
                        {['Accepted', 'Processing', 'Shipped', 'Delivered'].map(st => (
                           <button key={st} onClick={() => setSelectedOrder({...selectedOrder, status: st})} className={`px-5 py-2.5 rounded-full text-[9px] font-bold uppercase transition-all ${selectedOrder.status === st ? 'bg-[var(--indian-maroon)] text-white shadow-lg' : 'bg-gray-50 text-gray-400'}`}>{st}</button>
                        ))}
                     </div>
                     <button onClick={() => handleUpdateOrderStatus(selectedOrder._id, selectedOrder.status, selectedOrder.courier, selectedOrder.trackingId)} className="w-full bg-[var(--indian-midnight)] text-[var(--indian-gold)] py-5 rounded-full font-bold uppercase text-[10px] tracking-[0.3em] shadow-xl">Confirm Registry Update</button>
                  </div>
               </div>
            </div>
         </div>
